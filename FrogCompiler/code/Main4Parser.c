...
extern frog_void startParser(frog_void);
extern frog_int startScanner(BufferPointer sc_buf);

static frog_void displayParser(BufferPointer ptrBuffer);
static frog_long getParserFileSize(frog_char* fname);
static frog_void callGarbageCollector(frog_void);
...
frog_long getParserFileSize(frog_char* fname) {
	FILE* input;
	frog_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
frog_void displayParser(BufferPointer ptrBuffer) {
